...
The entry point is the `platform/argocd/` ArgoCD kustomization. This directory contains the Root Application (kustomization) that:

1. Points to `platform/argocd/apps/`
2. Deploys the `Application`/`ApplicationSet` definitions found there.
...
1. **Infrastructure** (`platform/argocd/apps/infrastructure-appset.yaml`): Core system components (Cilium, networking, CSI drivers, Cert-Manager). The apps referenced here point to folders under `infra/` (for example `infra/storage/csi-driver-proxmox`, `infra/networking/cilium`, etc.).
2. ~~**Monitoring** (`monitoring-appset.yaml`): Observability stack (Prometheus, Grafana).~~ To be implemented.
3. ~~**My Apps** (`my-apps-appset.yaml`): User workloads.~~ To be implemented.
...
| **1** | **Storage** | `csi-driver-proxmox`, `csi-driver-nfs`, `local-storage` | **Persistence**. Depends on Wave 0 for networking and secret access so CSI drivers and storage classes can provision PVCs. |
| **2** | **System** | `cert-manager`, `databases` | **Core Services**. Depends on Storage (PVCs) and Networking (Ingress/Gateway). |
...
Each `Application` resource in `platform/argocd/apps/` (for example `platform/argocd/apps/csi-driver-proxmox-app.yaml`, `platform/argocd/apps/root.yaml`) is annotated with a sync wave. The `path` fields in those `Application`/`ApplicationSet` manifests point to the literal repo directories like `infra/storage/csi-driver-proxmox`.
...
To fix this, we inject a custom Lua health check in `platform/argocd/values.yaml` (the ArgoCD values used to customize health checks).
...
1. **Bootstrap**: You apply the Root Application from `platform/argocd/` (kustomization/root Application).
...
5. **Storage**: ArgoCD deploys `proxmox-csi-plugin` (Wave 1).
